...
exports.addPost = (data, callback) => {
    ...
    (error, results, fields) => {
        ...
        return callback(null, "Post added Successfuly");
        }
    );
};

exports.getAllPosts = (data, callback) => {
    db.query('SELECT p.id AS postId, p.description, p.datetimeCreated, p.likeCount, p.dislikeCount, p.addedByUserId, u.firstName, u.lastName FROM posts AS p INNER JOIN users AS u ON p.addedByUserId = u.id',
    [],
    (error, results, fields) => {
        if (error){
            return callback(error);
